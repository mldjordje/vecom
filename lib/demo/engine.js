...
export function mount(DATA) {
  ...
  var CU_NAV = [
    { href: "#/app", icon: "dash", label: "Početna" },
    { href: "#/app/devices", icon: "zap", label: "Moji aparati" },
    { href: "#/app/edu", icon: "grad", label: "Edukacija" },
    { href: "#/app/tickets", icon: "wrench", label: "Servis" },
    { href: "#/app/parts", icon: "cart", label: "Potrošni" },
    { href: "#/app/protocols", icon: "book", label: "Protokoli" },
    { href: "#/app/docs", icon: "file", label: "Dokumenta" },
    { href: "#/app/news", icon: "news", label: "Novosti" },
  ];
  var CU_TABS = [
    { href: "#/app", icon: "dash", label: "Početna" },
    { href: "#/app/edu", icon: "grad", label: "Edukacija" },
    { href: "#/app/tickets", icon: "wrench", label: "Servis" },
    { href: "#/app/parts", icon: "cart", label: "Potrošni" },
    { href: "#/app/more", icon: "more", label: "Više" },
  ];
  ...
  function customerShell(current, html, o) {
    ...
    var nav = CU_NAV.map(function (n) {
      ...
      return '<a href="' + n.href + '" class="' + (n.href === current ? "on" : "") + '">' + ico(n.icon) +
        "<span>" + esc(n.label) + "</span>" + (b ? '<i class="cu-badge">' + b + "</i>" : "") + "</a>";
    }).join("");
    ...
    return '<div class="cu">' +
      ...
      '<div class="cu-help"><div class="tiny muted">Vecom podrška</div><div class="small">' + VECOM_PHONE_LABEL + "</div>" +
      '<div class="row" style="gap:6px;margin-top:8px"><a class="btn sm grow" href="tel:' + VECOM_PHONE + '">Pozovi</a>' +
      '<a class="btn ghost sm grow" href="viber://chat?number=%2B381692296005">Viber</a></div></div>' +
      '<button class="cu-logout" data-logout="1">' + ico("logout", 16) + "<span>Odjava</span></button>" +
      "</aside>" +
      '<section class="cu-main">' +
      '<header class="cu-top"><div class="grow"><div class="eyebrow">' + esc(o.eyebrow || c.name + " · " + c.city) + "</div>" +
      "<h1>" + esc(o.title || (CU_NAV.filter(function (n) { return n.href === current; })[0] || {}).label || "") + "</h1></div>" +
      (o.action || '<a class="btn sm cu-top-cta" href="#/app/fault">' + ico("alert", 15) + " Prijavi kvar</a>") +
      "</header>" +
      '<div class="cu-content">' + html + "</div>" +
      "</section>" +
      '<nav class="cu-tabs">' + CU_TABS.map(function (n) {
        var on = n.href === current || (n.href === "#/app/more" && ["#/app/devices", "#/app/protocols", "#/app/docs", "#/app/news", "#/app/more"].indexOf(current) >= 0);
  ...
  function warrantyRing(inst) {
    ...
    if (left < 0) return ring(100, "alert", "0 d", "istekla");
    return ring((left / total) * 100, left < 60 ? "warn" : "ok", left + " d", "garancija");
  }

  function probeRing(inst) {
    var pct = Math.round((inst.probe.used / inst.probe.total) * 100);
    return ring(pct, pct >= 85 ? "alert" : pct >= 70 ? "warn" : "ok", pct + "%", inst.probe.name.indexOf("Sonda") === 0 ? "sonda" : "ručica");
  }

  function deviceStatus(inst) {
    var open = myOpenTickets().filter(function (t) { return t.slug === inst.slug; })[0];
    if (open) return '<span class="pill warn"><i class="dot"></i>Servis u toku · #' + open.id + "</span>";
    if (daysUntil(inst.warrantyUntil) < 0) return '<span class="pill"><i class="dot"></i>Radi · van garancije</span>';
    return '<span class="pill ok"><i class="dot"></i>Radi normalno</span>';
  }
  ...
  function deviceCard(inst) {
    ...
    return '<button class="cu-dev" data-nav="#/app/device/' + esc(inst.slug) + '">' +
      ...
      '<div class="row spread wrap" style="gap:8px"><h3>' + esc(p ? p.name : inst.slug) + "</h3>" + deviceStatus(inst) + "</div>" +
      '<div class="tiny muted">' + esc(inst.serial) + " · šifra " + esc(inst.sku) + "</div>" +
  ...
  function miniSteps(t) {
    ...
    return '<div class="steps-h">' + TICKET_FLOW.map(function (st, i) {
      return '<div class="sh ' + (i < cur ? "done" : i === cur ? "now" : "") + '"><i></i><span>' + esc(st.label) + "</span></div>";
    }).join("") + "</div>";
  }
  ...
  function ticketCard(t) {
    ...
    return '<button class="cu-ticket" data-nav="#/app/ticket/' + t.id + '">' +
      ...
      '<div class="tiny muted">' + esc(p ? p.name : t.slug) + " · prijavljeno " + dmy(t.opened) + "</div></div>" +
      '<span class="pill ' + (t.status === "reseno" ? "ok" : "warn") + '"><i class="dot"></i>' + esc(flowLabel(t.status)) + "</span></div>" +
      miniSteps(t) +
      (last ? '<div class="cu-last"><span class="tiny muted">' + (last.from === "vecom" ? "Vecom servis" : "Vi") + ":</span> " + esc(last.text) + "</div>" : "") +
      "</button>";
  }

  // ticketRow ostaje za kompaktne liste
  ...
  function activity() {
    ...
    DATA.demo.installations.forEach(function (i) {
      var p = product(i.slug), nm = p ? p.name : i.slug;
      ev.push({ d: i.delivered, icon: "zap", t: "Isporučen i pušten u rad: " + nm });
      i.trained.forEach(function (x) { ev.push({ d: x.date, icon: "users", t: "Obuka završena: " + x.name + " · " + nm }); });
      i.service.forEach(function (s) { ev.push({ d: s.date, icon: "check", t: s.title + " · " + nm }); });
    });
    myTickets().forEach(function (t) { ev.push({ d: t.opened, icon: "alert", t: "Prijavljen kvar #" + t.id + ": " + t.issue }); });
    ev.push({ d: "2026-06-12", icon: "box", t: "Porudžbina #1198 isporučena" });
  ...
  function postCard(p, href) {
    return '<a class="cu-post" href="' + href + '">' +
      ...
      '<div class="cu-post-b"><div class="tiny muted">' + dmy(p.date) + (p.audience !== "all" ? " · za vlasnike " + esc((product(p.audience) || {}).name || "") : "") + "</div>" +
      "<h3>" + esc(p.title.sr) + "</h3>" +
      '<p class="small muted">' + esc(p.excerpt.sr) + "</p></div></a>";
  }

  function greeting() {
    var h = new Date().getHours();
    return h < 11 ? "Dobro jutro" : h < 18 ? "Dobar dan" : "Dobro veče";
  }
  ...
  function viewCustomerHome() {
    ...
    var quick = [
      { href: "#/app/fault", icon: "alert", t: "Prijavi kvar", s: "sa fotografijom" },
      { href: "#/app/parts", icon: "cart", t: "Potrošni", s: "sonda, filter, gel" },
      { href: "#/app/protocols", icon: "book", t: "Protokoli", s: "parametri tretmana" },
      { href: "tel:" + VECOM_PHONE, icon: "phoneCall", t: "Pozovi Vecom", s: VECOM_PHONE_LABEL },
    ];

    var html =
      '<section class="cu-hero">' +
      '<div class="cu-hero-t"><div class="eyebrow">' + esc(todayLong()) + "</div>" +
      "<h2>" + greeting() + ", " + esc(c.contact.split(" ")[0]) + ".</h2>" +
      '<p class="muted">' + inst.length + " aparata u radu" +
      (open.length ? " · " + open.length + " servis u toku" : " · sve radi") +
      " · sonda na Sonati je na " + probePct + "%.</p></div>" +
      ...
      (probePct >= 70
        ? '<div class="cu-alert">' + ico("alert", 18) + '<div class="grow"><div class="small">Sonda na Sonati 4XD je na ' + probePct + "% resursa</div>" +
          '<div class="tiny muted">Poručite rezervnu na vreme — isporuka traje 3–5 radnih dana.</div></div>' +
          '<button class="btn sm" data-quick-part="p1">Poruči sondu</button></div>'
        : "") +

      '<div class="cu-sec"><div class="row spread"><h3>Moji aparati</h3><a class="panel-link" href="#/app/devices">Svi aparati' + ico("chev", 14) + "</a></div>" +
      '<div class="cu-devs">' + inst.map(deviceCard).join("") + "</div></div>" +

      '<div class="cu-cols">' +
      '<div class="cu-col">' +
      '<article class="panel"><div class="row spread mb"><div><div class="eyebrow">Servis</div><h3>' + (open.length ? "U toku" : "Sve radi") + "</h3></div>" +
      '<a class="panel-link" href="#/app/tickets">Svi servisi' + ico("chev", 14) + "</a></div>" +
      (open.length ? '<div class="list">' + open.map(ticketCard).join("") + "</div>"
        : '<p class="small muted">Nema otvorenih prijava. Ako nešto ne radi, prijava traje minut.</p>') +
      "</article>" +
      '<article class="panel"><div class="eyebrow">Istorija</div><h3 class="mb">Aktivnost</h3>' + activityList(5) + "</article>" +
      "</div>" +
      '<div class="cu-col">' + courseCard() +
      '<article class="panel"><div class="row spread mb"><div><div class="eyebrow">Od Vecoma</div><h3>Novosti</h3></div>' +
      '<a class="panel-link" href="#/app/news">Sve' + ico("chev", 14) + "</a></div>" +
      '<div class="list">' + posts.map(function (p) { return postCard(p, "#/app/news/" + p.slug); }).join("") + "</div></article>" +
      '<article class="panel cu-contact"><div class="eyebrow">Vaš kontakt u Vecomu</div>' +
      '<div class="row" style="margin:10px 0 12px"><span class="cu-av">JR</span><div><div class="small">' + esc(DATA.demo.admin.name) + "</div>" +
      '<div class="tiny muted">Prodaja i podrška · ' + esc(DATA.demo.admin.email) + "</div></div></div>" +
      '<div class="grid3" style="gap:6px"><a class="btn sm" href="tel:' + VECOM_PHONE + '">Pozovi</a>' +
      '<a class="btn ghost sm" href="viber://chat?number=%2B381692296005">Viber</a>' +
      '<a class="btn ghost sm" href="mailto:' + esc(DATA.demo.admin.email) + '">Mejl</a></div></article>' +
      "</div></div>";

    return customerShell("#/app", html, { title: "Početna" });
  }

  function viewDevices() {
    return customerShell("#/app/devices",
      '<div class="cu-devs">' + DATA.demo.installations.map(deviceCard).join("") + "</div>" +
      '<p class="small muted mt">Svaki aparat koji kupite od Vecoma automatski se pojavljuje ovde, sa garancijom, obukom i dokumentima.</p>',
      { title: "Moji aparati" });
  }

  function viewDevice(slug) {
    var inst = instBySlug(slug);
    if (!inst) return customerShell("#/app/devices", '<div class="card">Aparat nije pronađen.</div>', { title: "Aparat" });
    var p = product(slug);
    var tab = S.devTab || "pregled";
    var tabs = [["pregled", "Pregled"], ["servis", "Servis"], ["obuka", "Obuka"], ["spec", "Specifikacija"]];
    ...
    if (tab === "servis") {
      body = (tickets.length ? '<div class="list mb">' + tickets.map(ticketCard).join("") + "</div>" : "") +
        '<article class="panel"><h3 class="mb">Istorija servisa</h3><div class="cu-act">' + inst.service.map(function (s) {
          return '<div class="cu-act-i"><span class="cu-act-ic">' + ico("check", 15) + '</span><div class="grow"><div class="small">' + esc(s.title) + "</div>" +
            '<div class="tiny muted">' + dmy(s.date) + " · zatvoreno</div></div></div>";
        }).join("") + "</div></article>";
    } else if (tab === "obuka") {
      body = '<article class="panel"><h3 class="mb">Obučeni za rad na aparatu</h3><div class="list">' + inst.trained.map(function (x) {
        return '<div class="item" style="cursor:default"><span class="cu-av sm">' + esc(initials(x.name)) + '</span><div class="grow"><div class="small">' + esc(x.name) + "</div>" +
          '<div class="tiny muted">Sertifikat izdat ' + dmy(x.date) + "</div></div>" +
          '<button class="btn ghost sm" data-download="Sertifikat — ' + esc(x.name) + '">' + ico("download", 14) + " Sertifikat</button></div>";
      }).join("") + "</div>" +
        '<div class="cu-alert soft mt">' + ico("users", 18) + '<div class="grow"><div class="small">Nova koleginica u salonu?</div><div class="tiny muted">Zakažite obuku — sertifikat stiže ovde.</div></div>' +
        '<button class="btn ghost sm" data-toast="Zahtev za obuku poslat Vecomu">Zakaži obuku</button></div></article>';
    } else if (tab === "spec") {
      ...
      }).join("") : '<p class="small muted">Specifikacija nije dostupna.</p>') + "</article>";
    } else {
      body = '<div class="cu-cols"><div class="cu-col"><article class="panel"><h3 class="mb">Podaci</h3><div class="kv">' +
        ...
        '<span class="k">Garancija do</span><span class="v">' + dmy(inst.warrantyUntil) + "</span>" +
        '<span class="k">' + esc(inst.probe.name) + '</span><span class="v">' + inst.probe.used.toLocaleString("sr-RS") + " / " + inst.probe.total.toLocaleString("sr-RS") + "</span>" +
        "</div></article></div>" +
        '<div class="cu-col"><article class="panel"><h3 class="mb">Brze akcije</h3><div class="cu-quick two">' +
        [["#/app/fault", "alert", "Prijavi kvar"], ["#/app/parts", "cart", "Poruči potrošni"], ["#/app/protocols", "book", "Protokoli"], ["#/app/docs", "file", "Dokumenta"]].map(function (q) {
          return '<a class="cu-q" href="' + q[0] + '"><span class="cu-q-ic">' + ico(q[1], 18) + '</span><span class="small">' + q[2] + "</span></a>";
        }).join("") + "</div></article>" +
        (daysUntil(inst.warrantyUntil) < 0
          ? '<div class="cu-alert mt">' + ico("shield", 18) + '<div class="grow"><div class="small">Garancija je istekla ' + dmy(inst.warrantyUntil) + "</div>" +
            '<div class="tiny muted">Servisni ugovor pokriva redovan servis i prioritet u redu.</div></div>' +
            '<button class="btn sm" data-toast="Zahtev za servisni ugovor poslat Vecomu">Ponuda</button></div>'
          : "") +
        "</div></div>";
    }

    var html =
      '<a class="small muted" href="#/app/devices">‹ Moji aparati</a>' +
      ...
      "<h2>" + esc(p ? p.name : slug) + "</h2>" +
      '<p class="small muted">' + esc(p ? p.desc : "") + "</p>" +
      '<div class="cu-rings">' + warrantyRing(inst) + probeRing(inst) + "</div>" +
      "</div></section>" +
      '<div class="seg mt">' + tabs.map(function (x) {
        return '<button class="' + (tab === x[0] ? "on" : "") + '" data-dev-tab="' + x[0] + '">' + x[1] + "</button>";
      }).join("") + "</div>" +
      '<div class="mt">' + body + "</div>";

    return customerShell("#/app/devices", html, { title: p ? p.name : "Aparat", eyebrow: inst.serial });
  }
  ...
  function viewCustomerNews(slug) {
    ...
    if (slug) {
      ...
      if (!p) return customerShell("#/app/news", '<div class="card">Objava nije pronađena.</div>', { title: "Novosti" });
      var body = (p.body.sr || []).map(function (b) {
        return b.style === "h2" || b.style === "h3" ? "<h3>" + esc(b.text) + "</h3>" : "<p>" + esc(b.text) + "</p>";
      }).join("");
      return customerShell("#/app/news",
        '<a class="small muted" href="#/app/news">‹ Sve novosti</a>' +
        '<article class="cu-article mt">' + (p.image ? '<img src="' + esc(p.image) + '" alt="">' : "") +
        '<div class="tiny muted">' + dmy(p.date) + "</div><h2>" + esc(p.title.sr) + "</h2>" +
        '<p class="lead">' + esc(p.excerpt.sr) + "</p>" + body + "</article>",
        { title: "Novosti", eyebrow: "Od Vecoma" });
    }
    return customerShell("#/app/news",
  ...
  function viewCustomerMore() {
    var links = [
      ["#/app/devices", "zap", "Moji aparati", "Garancija, sonda, servis, obuka"],
      ["#/app/protocols", "book", "Protokoli tretmana", "Parametri po problemu i fototipu"],
      ["#/app/docs", "file", "Dokumenta", "Uputstva, sertifikati, garantni list"],
      ["#/app/news", "news", "Novosti", "Novi protokoli i saveti od Vecoma"],
      ["tel:" + VECOM_PHONE, "phoneCall", "Pozovi Vecom", VECOM_PHONE_LABEL],
    ];
    return customerShell("#/app/more",
      '<div class="list">' + links.map(function (l) {
        return '<a class="item" href="' + l[0] + '" style="text-decoration:none"><span class="cu-q-ic">' + ico(l[1], 18) + "</span>" +
          '<div class="grow"><div class="small">' + l[2] + '</div><div class="tiny muted">' + esc(l[3]) + '</div></div><span class="chev">›</span></a>';
      }).join("") +
      '<button class="item" data-logout="1"><span class="cu-q-ic">' + ico("logout", 18) + '</span><div class="grow small">Odjava</div></button></div>',
      { title: "Više" });
  }
  ...
  function viewCustomerTickets() {
    ...
    return customerShell("#/app/tickets",
      (list.length ? '<div class="list">' + list.map(ticketCard).join("") + "</div>"
        : '<div class="card">Nema prijava.</div>') +
      '<p class="small muted mt">Status se menja čim servis nešto uradi — obaveštenje stiže i na mejl.</p>',
      { title: "Servis" });
  }
  ...
  function lessonRow(l, active) {
    ...
    return '<a class="lesson' + (done ? " done" : "") + (active ? " on" : "") + '" href="#/app/edu/' + l.n + '">' +
      ...
      '<span class="grow"><span class="small">' + esc(l.t) + '</span><span class="tiny muted">' + l.d + " min</span></span></a>";
  }
  ...
  function courseCard() {
    ...
    return '<article class="panel cu-course">' +
      '<div class="row" style="gap:16px;align-items:center">' + ring(pct, "ok", n + "/" + LESSONS.length, "lekcija") +
      '<div class="grow"><div class="eyebrow">' + esc(COURSE.school) + "</div><h3>" + esc(COURSE.title) + "</h3>" +
      '<div class="small muted">' + (courseDone() ? "Kurs završen — sertifikat je spreman." : "Sledeća: " + nl.n + ". " + esc(nl.t)) + "</div></div></div>" +
      '<div class="row wrap mt" style="gap:8px">' +
      (courseDone()
        ? '<a class="btn sm" href="#/app/edu/cert">' + ico("grad", 15) + " Sertifikat</a>"
        : '<a class="btn sm" href="#/app/edu/' + nl.n + '">' + ico("play", 15) + (n ? " Nastavi" : " Počni kurs") + "</a>") +
      '<a class="btn ghost sm" href="#/app/edu">Sve lekcije</a></div></article>';
  }
  ...
  function viewEdu() {
    ...
    var html =
      ...
      '<div class="grow"><div class="eyebrow">' + esc(COURSE.school) + " · online kurs</div>" +
      "<h2>" + esc(COURSE.title) + "</h2>" +
      '<p class="muted">' + LESSONS.length + " lekcija u " + COURSE.modules.length + " modula · video + tekst · sertifikat na kraju. Gledajte kad stignete, napredak se čuva.</p>" +
      '<div class="bar mt" style="max-width:420px"><i class="ok" style="width:' + pct + '%"></i></div>' +
      '<div class="tiny muted" style="margin-top:6px">' + n + " od " + LESSONS.length + " lekcija · " + pct + "% · kvizovi " + quizzesPassed() + "/" + QUIZ.length + "</div>" +
      '<div class="row wrap mt" style="gap:8px">' +
      (courseDone()
        ? '<a class="btn" href="#/app/edu/cert">' + ico("grad", 16) + " Preuzmi sertifikat</a>"
        : '<a class="btn" href="#/app/edu/' + nl.n + '">' + ico("play", 16) + (n ? " Nastavi: lekcija " + nl.n : " Počni kurs") + "</a>") +
      "</div></div>" +
      '<div class="edu-cert-teaser' + (courseDone() ? " open" : "") + '">' + ico("grad", 26) +
      '<div class="small">' + (courseDone() ? "Sertifikat otključan" : "Sertifikat se otključava posle svih lekcija i kvizova") + "</div>" +
      '<div class="tiny muted">Na ime polaznika, sa brojem sertifikata, spreman za štampu</div></div>' +
      "</section>" +
      '<div class="edu-modules">' + COURSE.modules.map(function (m, mi) {
        ...
        return '<article class="panel"><div class="row spread mb"><div><div class="eyebrow">Modul ' + (mi + 1) + "</div><h3>" + esc(m.title) + "</h3></div>" +
      ...
      '<p class="tiny muted mt">Demo: <button class="linkish" data-course-all="1">označi sve lekcije kao odgledane</button> — da se na sastanku odmah pokaže sertifikat.</p>';
    return customerShell("#/app/edu", html, { title: "Edukacija", eyebrow: COURSE.school });
  }

  function viewLesson(n) {
    var l = LESSONS[Number(n) - 1];
    if (!l) return viewEdu();
    var done = !!S.course.done[l.n];
    ...
    var player = l.video
      ...
      : '<div class="edu-player" id="edu-player">' +
        '<button class="edu-play" data-course-play="' + l.n + '" aria-label="Pusti lekciju">' + ico("play", 30) + "</button>" +
        '<div class="edu-player-t"><div class="small">Lekcija ' + l.n + " · " + esc(l.t) + "</div>" +
        '<div class="tiny">Klip ' + l.n + "/" + LESSONS.length + " · " + l.d + " min · video se snima</div></div>" +
        '<div class="edu-progress"><i id="edu-prog" style="width:' + (done ? 100 : 0) + '%"></i></div></div>';

    var html =
      '<a class="small muted" href="#/app/edu">‹ Svi moduli</a>' +
      '<div class="edu-layout mt">' +
      '<div class="edu-main">' + player +
      '<div class="row spread wrap mt" style="gap:10px"><div><div class="eyebrow">Modul ' + (l.mi + 1) + " · " + esc(l.module) + "</div>" +
      "<h2>" + l.n + ". " + esc(l.t) + "</h2></div>" +
      (done ? '<span class="pill ok"><i class="dot"></i>Odgledano</span>' : "") + "</div>" +
      '<p class="muted">' + esc(l.x) + "</p>" +
      '<article class="panel"><div class="eyebrow">Zapamtite</div><ul class="edu-keys">' +
      l.k.map(function (k) { return "<li>" + ico("check", 15) + "<span>" + esc(k) + "</span></li>"; }).join("") + "</ul></article>" +
      '<div class="row spread wrap mt" style="gap:8px">' +
      (prev ? '<a class="btn ghost sm" href="#/app/edu/' + prev.n + '">‹ ' + prev.n + ". lekcija</a>" : "<span></span>") +
      (done
        ? (!next || next.mi !== l.mi
          ? (quizState(l.mi).passed
            ? (next ? '<a class="btn sm" href="#/app/edu/' + next.n + '">Sledeća lekcija ›</a>' : '<a class="btn sm" href="#/app/edu/cert">' + ico("grad", 15) + " Sertifikat</a>")
            : '<a class="btn sm" href="#/app/edu/quiz/' + l.mi + '">Kviz modula ' + (l.mi + 1) + ' ›</a>')
          : '<a class="btn sm" href="#/app/edu/' + next.n + '">Sledeća lekcija ›</a>')
        : '<button class="btn sm" data-course-done="' + l.n + '">' + ico("check", 15) + " Završi lekciju</button>") +
      "</div></div>" +
      '<aside class="edu-side panel"><div class="row spread mb"><h3>Lekcije</h3><span class="pill">' + doneCount() + "/" + LESSONS.length + "</span></div>" +
  ...
  function quizRow(mi) {
    ...
    return '<a class="lesson quiz' + (st.passed ? " done" : "") + (open ? "" : " locked") + '" href="' + (open ? "#/app/edu/quiz/" + mi : "#/app/edu") + '">' +
      ...
      '<span class="grow"><span class="small">Kviz modula · 3 pitanja</span><span class="tiny muted">' +
      (st.passed ? "Položen · " + st.score + "/3" : open ? "Spreman" : "Otključava se posle lekcija") + "</span></span></a>";
  }
  ...
  function viewQuiz(mi) {
    ...
    if (!moduleLessonsDone(mi)) {
      return customerShell("#/app/edu", '<div class="card center" style="max-width:520px;margin:0 auto"><h2>Kviz je zaključan</h2>' +
        '<p class="small muted">Prvo odgledajte sve lekcije modula ' + (mi + 1) + ".</p>" +
        '<a class="btn" href="#/app/edu">Nazad na kurs</a></div>', { title: "Kviz", eyebrow: COURSE.school });
    }
    var st = quizState(mi);
    ...
    var html =
      '<a class="small muted" href="#/app/edu">‹ Svi moduli</a>' +
      '<div class="quiz mt"><div class="eyebrow">Modul ' + (mi + 1) + " · " + esc(COURSE.modules[mi].title) + "</div>" +
      "<h2>Kviz — 3 pitanja</h2>" +
      '<p class="small muted">Za prolaz su potrebna 2 tačna odgovora. Možete ponavljati koliko god treba.</p>' +
      qs.map(function (q, qi) {
        ...
        return '<article class="panel quiz-q"><div class="small"><b>' + (qi + 1) + ".</b> " + esc(q.q) + "</div>" +
          ...
          (st.checked ? '<div class="tiny ' + (pick === q.a ? "quiz-ok" : "quiz-bad") + '">' + (pick === q.a ? "Tačno. " : "Netačno. ") + esc(q.x) + "</div>" : "") +
          "</article>";
      }).join("") +
      (st.checked
        ? '<div class="quiz-result ' + (st.passed ? "ok" : "bad") + '">' + ico(st.passed ? "check" : "alert", 18) +
          '<div class="grow"><div class="small">' + (st.passed ? "Položeno — " : "Nije položeno — ") + st.score + " od 3 tačno</div>" +
          '<div class="tiny muted">' + (st.passed ? (mi < QUIZ.length - 1 ? "Sledeći modul je otključan." : "Svi moduli su gotovi.") : "Pogledajte objašnjenja i pokušajte ponovo.") + "</div></div>" +
          (st.passed
            ? (courseDone() ? '<a class="btn sm" href="#/app/edu/cert">Sertifikat</a>' : '<a class="btn sm" href="#/app/edu">Nastavi kurs</a>')
            : '<button class="btn sm" data-quiz-retry="' + mi + '">Ponovi kviz</button>') +
          "</div>"
        : '<button class="btn mt" data-quiz-check="' + mi + '"' + (allPicked ? "" : " disabled") + ">Proveri odgovore</button>") +
      "</div>";
    return customerShell("#/app/edu", html, { title: "Kviz", eyebrow: COURSE.school });
  }
    ...
    return '<div class="login-wrap" style="max-width:480px"><div class="card center">' +
      ...
      (ok
        ? '<div class="bk-check" style="margin:0 auto 10px">' + ico("check", 28) + "</div>" +
          "<h2>Sertifikat je važeći</h2>" +
          '<div class="kv mt" style="text-align:left">' +
          '<span class="k">Broj</span><span class="v">' + esc(id) + "</span>" +
          '<span class="k">Polaznik</span><span class="v">' + esc(c.contact) + "</span>" +
          '<span class="k">Salon</span><span class="v">' + esc(c.name + ", " + c.city) + "</span>" +
          '<span class="k">Kurs</span><span class="v">' + esc(COURSE.title) + "</span>" +
          '<span class="k">Izdat</span><span class="v">' + dmy(new Date().toISOString().slice(0, 10)) + "</span>" +
          "</div>"
        : '<h2>Sertifikat nije pronađen</h2><p class="small muted">Broj ' + esc(id) + " ne postoji ili kurs još nije završen.</p>") +
      '<p class="tiny muted mt">Proveru izdaje ' + esc(COURSE.school) + " · DEMO</p>" +
      "</div></div>";
  }
  ...
  function viewCertificate() {
    if (!courseDone()) {
      return customerShell("#/app/edu",
        ...
        '<h2 class="mt">Sertifikat još nije otključan</h2>' +
        '<p class="small muted">Lekcije: ' + doneCount() + "/" + LESSONS.length + " · kvizovi: " + quizzesPassed() + "/" + QUIZ.length + ". Sertifikat se otključava kad je sve gotovo.</p>" +
        '<a class="btn" href="#/app/edu/' + nextLesson().n + '">Nastavi kurs</a></div>',
        { title: "Sertifikat", eyebrow: COURSE.school });
    }
    var c = DATA.demo.company;
    var html =
      '<div class="row spread wrap mb no-print" style="gap:8px"><a class="small muted" href="#/app/edu">‹ Edukacija</a>' +
      '<button class="btn" data-print="1">' + ico("download", 16) + " Štampaj / sačuvaj PDF</button></div>" +
      '<div class="cert-wrap"><div class="cert">' +
      '<img class="cert-logo" src="/assets/vecom-logo.png" alt="Vecom">' +
      '<div class="cert-kicker">' + esc(COURSE.school) + "</div>" +
      "<h1>Sertifikat o završenoj obuci</h1>" +
      '<p class="cert-lead">Ovim se potvrđuje da je</p>' +
      '<div class="cert-name">' + esc(c.contact) + "</div>" +
      '<p class="cert-lead">' + esc(c.name) + ", " + esc(c.city) + "</p>" +
      '<p class="cert-body">uspešno završila online kurs<br><b>„' + esc(COURSE.title) + "”</b><br>" +
      LESSONS.length + " lekcija · " + COURSE.modules.length + " modula · kvizovi " + quizScore() + "/" + (QUIZ.length * 3) + " tačno</p>" +
      '<div class="cert-foot">' +
      ...
      '<div><div class="cert-line"></div><div class="tiny">Datum: ' + dmy(new Date().toISOString().slice(0, 10)) + "</div></div>" +
      "</div>" +
      '<div class="cert-qr"><div id="cert-qr"></div><span>Provera sertifikata</span></div>' +
      '<div class="cert-demo">DEMO — primer sertifikata iz prototipa, nije važeći dokument</div>' +
      "</div></div>";
    return customerShell("#/app/edu", html, { title: "Sertifikat", eyebrow: COURSE.school });
  }
  ...
  function finishLesson(n) {
    ...
    toast(courseDone() ? "Kurs završen — sertifikat je otključan" : "Lekcija " + n + " odgledana");
    route();
  }
  ...
  var ADMIN_ROLES = {
    prodaja: {
      ...
      nav: [
        ...
        { href: "#/admin/content", icon: "users", label: "Iskustva i pitanja", group: "Sajt" },
      ],
  ...
  function markPersona(a) {
    var map = { site: 0, guide: 0, roi: 0, verify: 0, login: 1, app: 1, admin: 2 };
  ...
  document.addEventListener("click", function (e) {
    var hit = e.target.closest("[data-go],[data-nav],[data-guide],[data-guide-back],[data-guide-reset]," +
      ...
      "[data-content-lang],[data-content-save],[data-ct-add]");
    if (!hit) return;
    var d = hit.dataset;
    ...
    if (d.courseAll) {
      ...
      toast("Sve lekcije i kvizovi označeni — sertifikat je otključan");
      return route();
    }
    if (d.quizPick) {
    ...
    if (d.quizCheck) {
      ...
      toast(qst.passed ? "Kviz položen — " + qst.score + "/3" : "Nije položeno — " + qst.score + "/3");
      return route();
    }
