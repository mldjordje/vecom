...
export function mount(DATA) {
  ...
  var S = {
    ...
    fleetFilter: null,
    devTab: null,
    course: { done: { 1: true, 2: true, 3: true } },
    posts: initialPosts(),
    newsFilter: "all",
  ...
  function courseDone() { return doneCount() === LESSONS.length; }
  ...
  function viewEdu() {
    ...
    var html =
      ...
      '<div class="tiny muted" style="margin-top:6px">' + n + " od " + LESSONS.length + " lekcija · " + pct + "%</div>" +
      '<div class="row wrap mt" style="gap:8px">' +
      (courseDone()
      ...
      '<div class="small">' + (courseDone() ? "Sertifikat otključan" : "Sertifikat se otključava posle poslednje lekcije") + "</div>" +
      ...
      '<div class="edu-modules">' + COURSE.modules.map(function (m, mi) {
        ...
        return '<article class="panel"><div class="row spread mb"><div><div class="eyebrow">Modul ' + (mi + 1) + "</div><h3>" + esc(m.title) + "</h3></div>" +
          ...
          '<div class="lessons">' + ls.map(function (l) { return lessonRow(l, false); }).join("") + "</div></article>";
      }).join("") + "</div>" +
  ...
  function viewCertificate() {
    if (!courseDone()) {
      return customerShell("#/app/edu",
        ...
        '<p class="small muted">Odgledano ' + doneCount() + " od " + LESSONS.length + " lekcija. Sertifikat se otključava posle poslednje.</p>" +
    ...
    var html =
      ...
      LESSONS.length + " lekcija · " + COURSE.modules.length + " modula · rad sa aparatom, bezbednost, protokoli i održavanje</p>" +
      '<div class="cert-foot">' +
  ...
  var ADMIN_ROLES = {
    prodaja: {
      ...
      nav: [
        ...
        { href: "#/admin/news", icon: "news", label: "Novosti" },
  ...
  function viewAdminMore() {
    var links = [
      ["#/admin/fleet", "map", "Aparati u terenu", "Prodajna lista po garanciji i sondi"],
      ["#/admin/orders", "box", "Porudžbine", waitingOrders().length + " čeka potvrdu"],
      ["#/admin/stats", "chart", "Brojke", "Upiti po aparatu, zemlji i izvoru"],
    ];
    var body = '<div class="list">' + links.map(function (x) {
      return '<a class="item" href="' + x[0] + '" style="text-decoration:none"><span class="cu-q-ic">' + ico(x[1], 18) + "</span>" +
        '<div class="grow"><div class="small">' + x[2] + '</div><div class="tiny muted">' + esc(x[3]) + '</div></div><span class="chev">›</span></a>';
    }).join("") + "</div>";
  ...
  function route() {
    ...
    if (a === "site") {
      ...
      else if (b === "edu") html = c === "cert" ? viewCertificate() : c ? viewLesson(c) : viewEdu();
  ...
  document.addEventListener("click", function (e) {
    var hit = e.target.closest("[data-go],[data-nav],[data-guide],[data-guide-back],[data-guide-reset]," +
      ...
      "[data-news-publish],[data-news-schedule],[data-news-draft],[data-ne-cmd]");
    if (!hit) return;
    var d = hit.dataset;
    ...
    if (d.courseAll) { LESSONS.forEach(function (l) { S.course.done[l.n] = true; }); toast("Sve lekcije označene — sertifikat je otključan"); return route(); }
